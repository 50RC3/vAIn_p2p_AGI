...
class AdaptiveCompression:
    ...
    async def compress_model_updates(self, updates: Dict[str, Any]) -> Tuple[Dict[str, Any], float]:
        ...
        try:
            ...
            compressed_size = 0
            for _, item in compressed.items():
    ...
    async def decompress_model_updates(
        self,
    ...
    def _adjust_compression_rate(self) -> None:
        ...
        if self.last_quality < self.quality_threshold:
            ...
            logger.debug("Quality below threshold (%.2f < %.2f). Adjusting compression rate: %.2f -> %.2f",
                         self.last_quality, self.quality_threshold, self.current_rate, new_rate)
