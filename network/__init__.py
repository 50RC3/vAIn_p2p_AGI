...
class ConnectionMetrics:
    ...
    def get_health_score(self) -> float:
        ...
        # Calculate base score considering errors and reconnect attempts
        base_score = max(0.0, 1.0 - (self.error_count / 10.0) - (self.reconnect_attempts / 5.0))
        
        # Adjust score based on recent activity
        age = time.time() - self.created_at
        idle_time = time.time() - self.last_used
        
        # Connections used more recently are healthier
        recency_factor = max(0.0, 1.0 - (idle_time / (24 * 3600)))  # 24 hour baseline
        
        return base_score * 0.7 + recency_factor * 0.3
...
class ConnectionPool:
    def __init__(self, max_connections: int = 100, ttl: int = 300, 
        ...
        self.retry_config = {
            ...
            'max_delay': 10,
            'backoff_factor': 2,
            'run_diagnostics': True  # Enable diagnostics on repeated failures
        }
    ...
    async def _create_connection(self, peer_id: str, custom_timeouts: Optional[Dict[str, float]] = None) -> Optional[Connection]:
        ...
        while retries < self.retry_config['max_retries']:
            try:
                ...
                conn.metrics.record_error("timeout")
            except socket.gaierror as e:
                last_error = f"DNS resolution error: {e}"
                conn.metrics.record_error("dns")
            except ConnectionRefusedError:
                last_error = "Connection refused"
                conn.metrics.record_error("refused")
            except OSError as e:
                if e.errno == errno.ENETUNREACH:
                    last_error = "Network unreachable"
                    conn.metrics.record_error("unreachable")
                else:
                    last_error = f"Network error: {e}"
                    conn.metrics.record_error("other")
            except Exception as e:
                last_error = str(e)
                conn.metrics.record_error("unknown")
        ...
        # Run diagnostics if enabled and we've exhausted all retries
        if self.retry_config['run_diagnostics'] and self.interactive:
            logger.info(f"Running network diagnostics for connection to {peer_id}")
            try:
                diagnostics = NetworkDiagnostics()
                asyncio.create_task(diagnostics.run_diagnostics(peer_id, 8000))
    ...
    async def _validate_connection(self, conn: Connection) -> bool:
        ...
        try:
            # Enhanced health check that detects more types of connection issues
            ...
            # Simple ping to verify connection is responsive
            conn.writer.write(b"ping")
            await asyncio.wait_for(conn.writer.drain(), timeout=2.0)
            return True
        except (ConnectionError, BrokenPipeError, asyncio.TimeoutError) as e:
            logger.warning(f"Connection to {conn.peer_id} unhealthy: {str(e)}")
            conn.is_healthy = False
            conn.metrics.record_error("health_check_failed")
            return False
        except Exception as e:
            logger.error(f"Unexpected error validating connection to {conn.peer_id}: {str(e)}")
            conn.is_healthy = False
            conn.metrics.record_error("validation_error")
            return False
    ...
    def _get_failover_peers(self, failed_peer: str) -> List[str]:
        """Get list of failover peers for a failed peer with health prioritization"""
        # Get all available peers except the failed one
        peers = [p for p in self.connections.keys() if p != failed_peer]
        
        if not peers:
            return []
            
        # Sort peers by health score (descending)
        return sorted(
            peers,
            key=lambda p: self.connections[p].metrics.get_health_score() if p in self.connections else 0.0,
            reverse=True
        )
